...
def main():
    ...
    # Hyperparameter tuning: learning rate η & momentum m
    param_grid = [
        (0.001, 0.2),
        (0.001, 0.6),
        (0.05,  0.6),
    ...
    print("\nHyperparameter CV Results:")
    print(" η      m     CE_loss±σ      MSE_loss±σ     Acc±σ")

    for lr, mom in param_grid:
        ...
        for train_idx, test_idx in skf.split(x_tensor, y_tensor):
            x_tr = x_tensor[train_idx].to(device)
            y_tr = y_tensor[train_idx].to(device)
            x_te = x_tensor[test_idx].to(device)
            y_te = y_tensor[test_idx].to(device)

            model = SimpleNet(x_tr.shape[1]).to(device)
            ...
            tr_l, val_l, tr_a, val_a = train(
                ...
                patience=201, min_delta=0.0
            )

            hist_tr_loss.append(tr_l)
            hist_val_loss.append(val_l)
            hist_tr_acc.append(tr_a)
            hist_val_acc.append(val_a)

            # last validation CE loss
            ce_list.append(val_l[-1])

            # MSE on one‐hot predictions
            with torch.no_grad():
                probs = F.softmax(model(x_te), dim=1)
        ...
        print(f"{lr:<6} {mom:<5} "
            f"{mean_ce:.4f}±{std_ce:.4f}   "
            f"{mean_mse:.4f}±{std_mse:.4f}   "
            f"{mean_acc:.4f}±{std_acc:.4f}")

        # convergence plots (mean over folds)
        ...
        plot_mean_loss_curves(mn_tr_loss, mn_val_loss, f"loss_eta{lr}_m{mom}.png")
        ...
        plot_mean_accuracy_curves(mn_tr_acc, mn_val_acc, f"acc_eta{lr}_m{mom}.png")
