...
def main():
    """Run training & 5‑fold CV with L2 regularization tuning."""
    sns.set_theme(style="whitegrid")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # --- load & preprocess as in A3 ---
    ...
    # --- L2 regularization tuning (r = weight_decay) ---
    param_r = [0.0001, 0.001, 0.01]
    print("\nRegularization CV Results (L2):")
    print("   r      CE_loss±σ      MSE_loss±σ     Acc±σ")
    ...
    for r in param_r:
        ...
        for train_idx, test_idx in skf.split(x_tensor, y_tensor):
            ...
            optimizer = optim.SGD(
                model.parameters(),
                lr=0.2, momentum=0.6,
                weight_decay=r         # L2 regularization term
            )
        ...
        print(f"{r:<6} {mean_ce:.4f}±{std_ce:.4f}   "
        ...
        plot_mean_loss_curves(mn_tr_loss, mn_val_loss, f"loss_r{r}.png")
        ...
        plot_mean_accuracy_curves(mn_tr_acc, mn_val_acc, f"acc_r{r}.png")
