...
def plot_class_distribution(y, save_path):
    ...
    classes = [f"Class {i}" for i in range(len(counts))]
    ...
    plt.title('Class Distribution', fontsize=18, fontweight='bold', pad=20)
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_correlation_heatmap(df, numeric_cols, save_path):
    ...
    # Draw the heatmap WITHOUT the mask to show full matrix
    sns.heatmap(
        corr, 
        cmap=cmap, 
        annot=True,
        ...
        vmax=1, 
        cbar_kws={"shrink": .8},
        annot_kws={"size": 9, "color": "black"}  # Force black text for visibility
    )
...
def main():
    ...
    # EDA plots BEFORE scaling/encoding
    plot_numeric_distributions_subplot(x_df, cont_cols + ord_cols, "feature_distributions_before.png")
    plot_correlation_heatmap(x_df, cont_cols + ord_cols, "correlation_heatmap.png")
    plot_class_distribution(y_np, "class_distribution.png")
